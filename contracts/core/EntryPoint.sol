...
contract EntryPoint is IEntryPoint, StakeManager {
    ...
    function handleOps(UserOperation[] calldata ops, address payable beneficiary) public {
        ...
        unchecked {
            for (uint256 i = 0; i < opslen; i++) {
                ...
                (uint256 sigTimeRange, uint256 paymasterTimeRange, ) = _validatePrepayment(
                    i,
                    ops[i],
                    opInfo,
                    address(0)
    ...
    function innerHandleOp(
        ...
        if (callData.length > 0) {
            (bool success, bytes memory result) = address(mUserOp.sender).call{gas: callGasLimit}(
                callData
            );
            if (!success) {
                if (result.length > 0) {
    ...
    function simulateValidation(UserOperation calldata userOp) external {
        ...
        (bool sigFailed, uint64 validAfter, uint64 validUntil) = _intersectTimeRange(
            sigTimeRange,
            paymasterTimeRange
        );
    ...
    function _createSenderIfNeeded(
        ...
        if (initCode.length != 0) {
            ...
            if (sender.code.length != 0)
                revert FailedOp(opIndex, address(0), "AA10 sender already constructed");
            address sender1 = senderCreator.createSender{gas: opInfo.mUserOp.verificationGasLimit}(
                initCode
            );
            if (sender1 == address(0))
                revert FailedOp(opIndex, address(0), "AA13 initCode failed or OOG");
            if (sender1 != sender)
                revert FailedOp(opIndex, address(0), "AA14 initCode must return sender");
            if (sender1.code.length == 0)
                revert FailedOp(opIndex, address(0), "AA15 initCode must create sender");
    ...
    function _intersectTimeRange(uint256 sigTimeRange, uint256 paymasterTimeRange)
        internal
        pure
        returns (
            bool sigFailed,
    ...
    {
        (sigFailed, validAfter, validUntil) = _parseSigTimeRange(sigTimeRange);
        (bool pmSigFailed, uint64 pmValidAfter, uint64 pmValidUntil) = _parseSigTimeRange(
            paymasterTimeRange
        );
